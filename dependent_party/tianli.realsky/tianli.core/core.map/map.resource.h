...
public:
    MapResource() = default;
    MapResource(const std::filesystem::path& path, std::string target_name, cv::Point map_origin, cv::Point origin_index) : map_origin(map_origin), origin_index(origin_index) {}
    ~MapResource() = default;
...
private:
    cv::Mat view(const cv::Rect& rect, const std::vector<cv::Point> index_s) { return view_abs(to_abs(rect), index_s); }
    ...
    cv::Mat view_async(const cv::Rect& rect, const std::vector<cv::Point> index_s)
    {
        ...
        for (auto& index : index_s)
        {
            ...
            cv::Rect r1 = r - blocks[index].rect.tl();
            cv::Rect r2 = r - rect.tl();
            futures.emplace_back(std::async(std::launch::async, [r1, r2, index, this, &map] { blocks[index].image(r1).copyTo(map(r2)); }));
        }
        for (auto& f : futures)
            f.get();
        return map;
    ...
    {
        return std::accumulate(blocks.begin(), blocks.end(), cv::Rect(0, 0, 16, 16), [](const cv::Rect& rect, const auto& block) { return rect | block.second.rect; });
    }
    ...
    std::vector<cv::Point> find_indexs_abs(const cv::Rect& rect)
    {
        ...
        std::ranges::copy_if(blocks | std::views::keys, std::back_inserter(indexs), [&](const cv::Point& index) { return (rect & blocks[index].rect).area() > 0; });
        return indexs;
    }
