...
namespace std
{
    template <> struct less<cv::Point>
    {
        bool operator()(const cv::Point& a, const cv::Point& b) const { return a.x < b.x || (a.x == b.x && a.y < b.y); }
    };
    template <> struct less<cv::Size>
    {
        bool operator()(const cv::Size& a, const cv::Size& b) const { return a.width < b.width || (a.width == b.width && a.height < b.height); }
    };
    template <> struct less<cv::Rect>
    {
