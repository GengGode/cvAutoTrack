...
namespace trackCache
{
    ...
    struct Setting
    {
        ...
        // 序列化模板
        template <class Archive> void serialize(Archive& ar) { ar(version, extended, up_right, octaves, octave_layers, hessian_threshold, roi.x, roi.y, roi.width, roi.height); }
    };
    ...
    private:
        ...
        struct _Sample_KeyPoint
        {
            ...
            int32_t class_id;
            // 序列化模板
            template <class Archive> void serialize(Archive& ar) { ar(x, y, response, class_id); }
        };
    ...
    public:
        ...
        template <class Archive> void save(Archive& ar) const
        {
            ...
            std::vector<char> descriptors_vector(descriptors_half.data, descriptors_half.data + descriptors_half.total() * 2);
#else
