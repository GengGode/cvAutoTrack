...
AutoTrack::AutoTrack()
{
	...
	wForAfter.append(this, &AutoTrack::getGengshinImpactWnd, 101, "æœªèƒ½æ‰¾åˆ°åŽŸç¥žçª—å£å¥æŸ„");
	wForAfter.append(this, &AutoTrack::getGengshinImpactRect, 102, "èŽ·å–åŽŸç¥žçª—å£å¤§å°å¤±è´¥");
	wForAfter.append(this, &AutoTrack::getGengshinImpactScreen, 103, "èŽ·å–åŽŸç¥žç”»é¢å¤±è´¥");
...
bool AutoTrack::GetVersion(char* version_buff, int buff_size)
{
	...
	{
		err = { 291,"»º´æÇøÎª¿ÕÖ¸Õë»òÊÇ»º´æÇø´óÐ¡ÎªÐ¡ÓÚ1" };
		return false;
	}
...
bool AutoTrack::DebugCapturePath(const char* path_buff, int buff_size)
{
	...
	{
		err = { 251,"Â·¾¶»º´æÇøÎª¿ÕÖ¸Õë»òÊÇÂ·¾¶»º´æÇø´óÐ¡ÎªÐ¡ÓÚ1" };
		return false;
	}
	...
	if (!rel)
	{
		err = { 252,std::string("±£´æ»­ÃæÊ§°Ü£¬Çë¼ì²éÎÄ¼þÂ·¾¶ÊÇ·ñºÏ·¨")+std::string(path_buff)};
		return false;
	}
	return true;
}
...
bool AutoTrack::GetTransformOfMap(double& x, double& y, double& a, int& mapId)
{
	...
	/*
	·Ö±ðÅÐ¶ÏÊÇ·ñ³É¹¦»ñÈ¡£¬±ÜÃâÇ°Ò»¸öerror_code±»ºóÒ»¸öerror_code¸²¸Ç
	¶øµ¼ÖÂ±¾º¯Êý·µ»Øfalse£¨±íÊ¾Ê§°Ü£©µ«error_codeÎª0£¨±íÊ¾³É¹¦£©¡£
	*/
...
bool AutoTrack::GetPosition(double& x, double& y)
{
	...
	if (capture->mode == Capture::Mode_Bitblt)
	{
		...
		{
			err = { 1001, "BitbltÄ£Ê½ÏÂ»ñÈ¡×ø±êÊ±£¬Ã»ÓÐÊ¶±ðµ½paimon£¬½¨ÒéÊ¹ÓÃDXÄ£Ê½" };
			return false;
		}
	}
	else
	{
		...
		{
			err = { 1002, "»ñÈ¡×ø±êÊ±£¬Ã»ÓÐÊ¶±ðµ½paimon" };
			return false;
		}
...
bool AutoTrack::GetDirection(double& a)
{
	...
	if (capture->mode == Capture::Mode_Bitblt)
	{
		if (getMiniMapRefMat_Bitblt() == false)
		{
			err = { 2001, "BitbltÄ£Ê½ÏÂ»ñÈ¡½ÇÉ«³¯ÏòÊ±£¬Ã»ÓÐÊ¶±ðµ½paimon" };
			return false;
		}
	}
	else
	{
		...
		if (!check_paimon(paimon_rect))
		{
			err = { 2002 ,"»ñÈ¡½ÇÉ«³¯ÏòÊ±£¬Ã»ÓÐÊ¶±ðµ½Paimon" };
			return false;
		}
	...
	std::vector<cv::Rect> boundRect(contours.size());  //¶¨ÒåÍâ½Ó¾ØÐÎ¼¯ºÏ
	cv::Point2f rect[4];
...
bool AutoTrack::GetRotation(double& a)
{
	...
	if (capture->mode == Capture::Mode_Bitblt)
	{
		if (getMiniMapRefMat_Bitblt() == false)
		{
			err = { 3001, "BitbltÄ£Ê½ÏÂ»ñÈ¡ÊÓ½Ç³¯ÏòÊ±£¬Ã»ÓÐÊ¶±ðµ½paimon" };
			return false;
		}
	}
	else
	{
		...
		if (!check_paimon(paimon_rect))
		{
			err = { 3002 ,"»ñÈ¡ÊÓ½Ç³¯ÏòÊ±£¬Ã»ÓÐÊ¶±ðµ½Paimon" };
			return false;
		}
	...
	{
		err = { 3003,"»ñÈ¡ÊÓ½Ç³¯ÏòÊ±£¬Ô­ÉñÐ¡µØÍ¼ÇøÓòÃ»ÓÐÈ¡µ½Í¸Ã÷Í¨µÀ" };
		return false;
	}

	if (capture->mode == Capture::Mode_DirectX)
	{
		err = { 3004,"DXÄ£Ê½ÏÂ£¬Ô­ÉñÐ¡µØÍ¼ÇøÓòÎÞ·¨È¡µ½Í¸Ã÷Í¨µÀ" };
		return false;
	}
	...
	if (contours.size() == 0)
	{
		err = { 3005 ,"»ñÈ¡ÊÓ½Ç³¯ÏòÊ±£¬Ã»ÓÐÌáÈ¡³öÊÓ½ÇÉÈÐÎÇøÓò" };
		return false;
	}
...
bool AutoTrack::GetStar(double& x, double& y, bool& isEnd)
{
	...
	else
	{
		...
		if (capture->mode == Capture::Mode_Bitblt)
		{
			if (getMiniMapRefMat_Bitblt() == false)
			{
				err = { 4001, "BitbltÄ£Ê½ÏÂ»ñÈ¡ÉñÍ«Ê±£¬Ã»ÓÐÊ¶±ðµ½paimon" };
				return false;
			}
		}
		else
		{
			...
			if (!check_paimon(paimon_rect))
			{
				err = { 4002, "»ñÈ¡ÉñÍ«Ê±£¬Ã»ÓÐÊ¶±ðµ½paimon" };
				return false;
			}
...
bool AutoTrack::getGengshinImpactWnd()
{
	if (is_Auto_getHandle)
	{
		...
		if (giHandle == NULL)
		{
			err = { 10,"ÎÞÐ§¾ä±ú»òÖ¸¶¨¾ä±úËùÖ¸Ïò´°¿Ú²»´æÔÚ" };
			return false;
		}
	}
	else
...
bool AutoTrack::getGengshinImpactRect()
{
	...
	// ×ó²àÒÑ»ñÈ¡ÎïÆ·¿ÉÄÜÐÔÇøÓò¼ÆËã²ÎÊý
	...
	// ÓÒ²à¿É¼ñÈ¡ÎïÆ·¿ÉÄÜÐÔÇøÓò¼ÆËã²ÎÊý
...
bool AutoTrack::getMiniMapRefMat_Bitblt()
{
	...
	// ¼ì²âÅÉÃÉ -> ¼ì²âÐ¡µØÍ¼±ê¶¨ -> ¼ÆËãÐ¡µØÍ¼×ø±ê
	...
	{
		err = { 40002,"BitbltÄ£Ê½ÏÂ¼ì²âÐ¡µØÍ¼±ê¶¨Ê§°Ü" };
		return false;
	}
	if (genshin_minimap_cailb.is_visial == false)
	{
		err = { 50002,"BitbltÄ£Ê½ÏÂÎ´ÄÜ¼ì²âµ½Ð¡µØÍ¼±ê¶¨" };
		return false;
	}
	...
	{
		err = { 40003, "BitbltÄ£Ê½ÏÂ¼ÆËãÐ¡µØÍ¼ÇøÓòÊ§°Ü" };
		return false;
	}
...
bool AutoTrack::getAvatarRefMat()
{
	if (giMiniMapRef.empty())
	{
		err = { 21,"»ñÈ¡½ÇÉ«ÇøÓòÊ±£¬Ð¡µØÍ¼ÇøÓòÎª¿Õ"};
		return false;
	}
