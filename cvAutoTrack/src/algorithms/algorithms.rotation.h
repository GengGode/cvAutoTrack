...
bool rotation_calculation(cv::Mat& giMiniMapRef, double& a);			//算法1，基于Alpha通道，高精度
bool rotation_calculation_2nd(cv::Mat& giMiniMapRef, double& a);	//算法2，基于屏幕空间，低精度

bool rotation_calculation(cv::Mat& giMiniMapRef, double& a)
{
  if (giMiniMapRef.channels() != 4)
    return rotation_calculation_2nd(giMiniMapRef, a);
  //取Alpha通道
...
bool rotation_calculation_2nd(cv::Mat& giMiniMapRef, double& a)
{
  cv::Point2i center(giMiniMapRef.size[1] / 2, giMiniMapRef.size[0] / 2);
  cv::Size2i miniMap_size = giMiniMapRef.size();
